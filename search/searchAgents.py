...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        successors = []
        x, y = state[0]
        visited = list(state[1]) #visited corners
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextX][nextY]:
                ...
                if nextPos in self.corners:
                    index = self.corners.index(nextPos) #corner index
                    newVisited[index] = True
                next = (nextPos, tuple(newVisited))
                successors.append((next, action, 1))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    distances = [util.manhattanDistance(current, corner) for corner in unvisited]
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    return max(max(distances), len(foodList))         # Return max(distance furthest food pellet, remaining no. food pellets
